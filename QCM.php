            ...
            <div class="card-fluid">
                <div class="card-body">
                    ...
                    <div data-aos="fade-up" data-aos-delay="600">
                        ...
                        if (!empty($search)) {
                            $sql = "SELECT COUNT(*) AS total FROM QCM WHERE 
                                    Titre_QCM LIKE :search OR 
                                    Description_QCM LIKE :search";
                        ...
                        $offset = ($page - 1) * $limit;
                        if (!empty($search)) {
                            $sql = "SELECT * FROM QCM WHERE 
                                    Titre_QCM LIKE :search OR 
                                    Description_QCM LIKE :search
                                    LIMIT :limit OFFSET :offset";
                        } else {
                            $sql = "SELECT * FROM QCM LIMIT :limit OFFSET :offset";
                        }
                        ...
                        <div class="row mb-3">
                                <div class="col-12 col-md-6 mb-2">
                                    <button id="addQCMBtn" class="btn btn-primary">
                        ...
                        <table class="table table-striped table-bordered">
                            ...
                            <tbody>
                                ...
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    echo "<tr>";
                                    echo "<td>".$row["Id_QCM"]."</td>"; // Displaying Id_QCM
                                    echo "<td>".$row["Id_Cours"]."</td>"; // Displaying Id_Cours
                                    echo "<td>".$row["Titre_QCM"]."</td>";
                                    echo "<td>".$row["Description_QCM"]."</td>";
                                    echo "<td>".$row["Solution_QCM"]."</td>";
                                    echo "<td>";
                                    echo "<a href='edit_qcm.php?id=".$row['Id_QCM']."' class='btn btn-outline-primary btn-sm me-1'><i class='bi bi-pencil'></i></a>";
                                    echo "<a href='delete_qcm.php?id=".$row['Id_QCM']."' class='btn btn-outline-danger btn-sm me-1'><i class='bi bi-trash'></i></a>";
                                    echo "<a href='afficher_qcm.php?id=".$row['Id_QCM']."' class='btn btn-outline-success btn-sm me-1'><i class='bi bi-eye'></i></a>";
                                    echo "</td>";
                                    echo "</tr>";
                                }
                                ?>
...
<div class="modal fade" tabindex="-1" role="dialog" id="addQCMModal">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <i class="far fa-list-alt fa-5x text-info"></i>
                    <h4>Découvrez notre nouveau QCM</h4>
                    </div>
                    <div class="col-md-6">
                    <?php
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                // Prepare SQL query to insert a new QCM into the database
                $sql = "INSERT INTO QCM (Titre_QCM, Description_QCM, Solution_QCM) 
                ...
                // Execute the query
                if ($stmt->execute()) {
                    // Redirect to a confirmation page or back to the list of QCMs
                    header("Location: qcm.php");
                    exit();
                } else {
                    echo "Erreur lors de l'ajout du QCM.";
                }
            }
            ?>
...
<script>
    document.getElementById("addQCMBtn").addEventListener("click", function() {
        $('#addQCMModal').modal('show');
    });
</script>
