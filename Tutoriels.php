    ...
    <div class="row">
        <div class="col-12 d-flex flex-column justify-content-center">
            <div class="card-fluid">
                <div class="card-body">
                    <h1 data-aos="fade-up" class="mb-4 display-4 fw-bold text-center text-primary">Liste des Tutoriels</h1>
                    <div data-aos="fade-up" data-aos-delay="600">
                        ...
                        if (!empty($search)) {
                            $sql = "SELECT COUNT(*) AS total FROM Tutoriels WHERE 
                                    Titre_Tutoriel LIKE :search OR 
                                    Description_Tutoriel LIKE :search";
                            $stmt = $pdo->prepare($sql);
                            $stmt->bindValue(':search', '%' . $search . '%', PDO::PARAM_STR);
                            $stmt->execute();
                        } else {
                            $sql = "SELECT COUNT(*) AS total FROM Tutoriels";
                            $stmt = $pdo->query($sql);
                        }
                        ...
                        $offset = ($page - 1) * $limit;
                        if (!empty($search)) {
                            $sql = "SELECT * FROM Tutoriels WHERE 
                                    Titre_Tutoriel LIKE :search OR 
                                    Description_Tutoriel LIKE :search
                                    LIMIT :limit OFFSET :offset";
                        } else {
                            $sql = "SELECT * FROM Tutoriels LIMIT :limit OFFSET :offset";
                        }
                        ...
                        <div class="row mb-3">
                                <div class="col-12 col-md-6 mb-2">
                                    <button id="addTutorielsBtn" class="btn btn-primary">
                                        <i class="bi bi-plus"></i>
                                        Ajouter un Tutoriels 
                                    </button>
                                </div>
                        ...
                        <table class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>Id_Tutoriel</th>
                                    <th>Id_Cours</th>
                                    <th>Titre_Tutoriel</th>
                                    <th>Description_Tutoriel</th>
                                    <th>Contenu_Tutoriel</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    echo "<tr>";
                                    echo "<td>".$row["Id_Tutoriel"]."</td>"; // Displaying Id_Tutoriel
                                    echo "<td>".$row["Id_Cours"]."</td>"; // Displaying Id_Cours
                                    echo "<td>".$row["Titre_Tutoriel"]."</td>";
                                    echo "<td>".$row["Description_Tutoriel"]."</td>";
                                    echo "<td>".$row["Contenu_Tutoriel"]."</td>";
                                    echo "<td>";
                                    echo "<a href='edit_tutoriel.php?id=".$row['Id_Tutoriel']."' class='btn btn-outline-primary btn-sm me-1'><i class='bi bi-pencil'></i></a>";
                                    echo "<a href='delete_tutoriel.php?id=".$row['Id_Tutoriel']."' class='btn btn-outline-danger btn-sm me-1'><i class='bi bi-trash'></i></a>";
                                    echo "<a href='afficher_tutoriel.php?id=".$row['Id_Tutoriel']."' class='btn btn-outline-success btn-sm me-1'><i class='bi bi-eye'></i></a>";
                                    echo "</td>";
                                    echo "</tr>";
                                }
                                ?>
...
<div class="modal fade" tabindex="-1" role="dialog" id="addTutorielsModal">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title">Bienvenue a apcpedagogie</h5>
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <i class="fas fa-lightbulb fa-5x text-info"></i>
                    <h4>Découvrez notre nouveau Tutoriels </h4>
                    </div>
                    <div class="col-md-6">
                    <?php
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                // Prepare SQL query to insert a new tutorial into the database
                $sql = "INSERT INTO Tutoriels (Titre_Tutoriel, Description_Tutoriel, Contenu_Tutoriel) 
                ...
                // Execute the query
                if ($stmt->execute()) {
                    // Redirect to a confirmation page or back to the list of tutorials
                    header("Location: tutoriels.php");
                    exit();
                } else {
                    echo "Erreur lors de l'ajout du tutoriel.";
                }
            }
            ?>
...
<script>
    document.getElementById("addTutorielsBtn").addEventListener("click", function() {
        $('#addTutorielsModal').modal('show');
    });
</script>
