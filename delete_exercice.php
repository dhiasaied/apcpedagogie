      ...
      <div class="card-fluid">
        <div class="card-body">
          ...
          <div data-aos="fade-up" data-aos-delay="600">
            ...
            // Vérifier si l'identifiant de l'exercice est passé en paramètre GET
            if (isset($_GET['id'])) {
              $id = $_GET['id'];

              // Préparez votre requête SQL pour récupérer les détails de l'exercice
              $sql = "SELECT * FROM Exercices WHERE Id_Exercice = :id";
              ...
              if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                // Affichez les détails de l'exercice dans un formulaire pour la confirmation de la suppression
                ?>
                <h2>Voulez-vous vraiment supprimer l'exercice suivant ?</h2>
                <table class="table table-striped table-bordered">
                  <thead>
                    <tr>
                      <th>Titre de l'exercice</th>
                      <th>Description de l'exercice</th>
                      <th>Solution de l'exercice</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td><?php echo $row['Titre_Exercice']; ?></td>
                      <td><?php echo $row['Description_Exercice']; ?></td>
                      <td><?php echo $row['Solution_Exercice']; ?></td>
                    </tr>
                  </tbody>
                </table>
                <form action="delete_confirm_exercices.php" method="POST">
                  <input type="hidden" name="id" value="<?php echo $row['Id_Exercice']; ?>">
                  <button type="submit" class="btn btn-danger">Confirmer la suppression</button>
                  <a href="exercices.php" class="btn btn-secondary">Annuler</a>
                </form>
            <?php
              } else {
                echo "Aucun exercice trouvé avec cet identifiant.";
              }
            } else {
              echo "Identifiant d'exercice non spécifié.";
            }
            ?>
          </div>
        </div>
