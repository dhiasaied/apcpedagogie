      ...
      <div class="card-fluid">
        <div class="card-body">
          ...
          <div data-aos="fade-up" data-aos-delay="600">
            ...
            // Vérifier si l'identifiant du tutoriel est passé en paramètre GET
            if (isset($_GET['id'])) {
              $id = $_GET['id'];

              // Préparez votre requête SQL pour récupérer les détails du tutoriel
              $sql = "SELECT * FROM Tutoriels WHERE Id_Tutoriel = :id";
              ...
              if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                // Affichez les détails du tutoriel dans un formulaire pour la confirmation de la suppression
                ?>
                <h2>Voulez-vous vraiment supprimer le tutoriel suivant ?</h2>
                <table class="table table-striped table-bordered">
                  <thead>
                    <tr>
                      <th>Titre du tutoriel</th>
                      <th>Description du tutoriel</th>
                      <th>Contenu du tutoriel</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td><?php echo $row['Titre_Tutoriel']; ?></td>
                      <td><?php echo $row['Description_Tutoriel']; ?></td>
                      <td><?php echo $row['Contenu_Tutoriel']; ?></td>
                    </tr>
                  </tbody>
                </table>
                <form action="delete_confirm_tutoriels.php" method="POST">
                  <input type="hidden" name="id" value="<?php echo $row['Id_Tutoriel']; ?>">
                  <button type="submit" class="btn btn-danger">Confirmer la suppression</button>
                  <a href="tutoriels.php" class="btn btn-secondary">Annuler</a>
                </form>
            <?php
              } else {
                echo "Aucun tutoriel trouvé avec cet identifiant.";
              }
            } else {
              echo "Identifiant de tutoriel non spécifié.";
            }
            ?>
          </div>
        </div>
