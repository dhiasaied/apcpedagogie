...
<div class="container-fluid" style="background-color: black; height: 300px;">
    ...
    <div class="row">
        <div class="col-lg-12 d-flex flex-column justify-content-center">
            <div class="card">
                <div class="card-body">
                    <h1 data-aos="fade-up" class="mb-4 display-4 fw-bold text-center text-primary">Liste des Exercices</h1>
                    <div data-aos="fade-up" data-aos-delay="600">
                        ...
                        if (!empty($search)) {
                            $sql = "SELECT COUNT(*) AS total FROM Exercices WHERE 
                                    Titre_Exercice LIKE :search OR 
                                    Description_Exercice LIKE :search";
                            $stmt = $pdo->prepare($sql);
                            $stmt->bindValue(':search', '%' . $search . '%', PDO::PARAM_STR);
                            $stmt->execute();
                        } else {
                            $sql = "SELECT COUNT(*) AS total FROM Exercices";
                            $stmt = $pdo->query($sql);
                        }
                        ...
                        $offset = ($page - 1) * $limit;
                        if (!empty($search)) {
                            $sql = "SELECT * FROM Exercices WHERE 
                                    Titre_Exercice LIKE :search OR 
                                    Description_Exercice LIKE :search
                                    LIMIT :limit OFFSET :offset";
                        } else {
                            $sql = "SELECT * FROM Exercices LIMIT :limit OFFSET :offset";
                        }
                        ...
                        ?>
                         <div class="row mb-3">
                                <div class="col-12 col-md-6 mb-2">
                                    <button id="addExercicesBtn" class="btn btn-primary">
                                        <i class="bi bi-plus"></i>
                                        Ajouter une Exercices
                                    </button>
                                </div>
                        ...
                        <table class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>Id_Exercice</th>
                                    <th>Id_Cours</th>
                                    <th>Titre_Exercice</th>
                                    <th>Description_Exercice</th>
                                    <th>Solution_Exercice</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    echo "<tr>";
                                    echo "<td>".$row["Id_Exercice"]."</td>"; // Displaying Id_Exercice
                                    echo "<td>".$row["Id_Cours"]."</td>"; // Displaying Id_Cours
                                    echo "<td>".$row["Titre_Exercice"]."</td>";
                                    echo "<td>".$row["Description_Exercice"]."</td>";
                                    echo "<td>".$row["Solution_Exercice"]."</td>";
                                    echo "<td>";
                                    echo "<a href='edit_exercice.php?id=".$row['Id_Exercice']."' class='btn btn-outline-primary btn-sm me-1'><i class='bi bi-pencil'></i></a>";
                                    echo "<a href='delete_exercice.php?id=".$row['Id_Exercice']."' class='btn btn-outline-danger btn-sm me-1'><i class='bi bi-trash'></i></a>";
                                    echo "<a href='afficher_exercice.php?id=".$row['Id_Exercice']."' class='btn btn-outline-success btn-sm me-1'><i class='bi bi-eye'></i></a>";
                                    echo "</td>";
                                    echo "</tr>";
                                }
                                ?>
...
<div class="d-flex justify-content-center"> 
  ...
  <!-- Modal -->
  <div class="modal fade" tabindex="-1" role="dialog" id="addExercicesModal">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <i class="fas fa-pencil-alt fa-5x text-info"></i>
                    <h4>Découvrez notre nouveau Exercice</h4>
                    </div>
                    <div class="col-md-6">
                    <?php
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                // Prepare SQL query to insert a new exercise into the database
                $sql = "INSERT INTO Exercices (Titre_Exercice, Description_Exercice, Solution_Exercice) 
                        VALUES (:titre, :description, :solution)";
                $stmt = $pdo->prepare($sql);
                ...
                // Execute the query
                if ($stmt->execute()) {
                    // Redirect to a confirmation page or back to the list of exercises
...
<script>
    document.getElementById("addExercicesBtn").addEventListener("click", function() {
        $('#addExercicesModal').modal('show');
    });
</script>
