      ...
      <div class="card-fluid">
        <div class="card-body">
          ...
          <div data-aos="fade-up" data-aos-delay="600">
            ...
            // Vérifier si l'identifiant du QCM est passé en paramètre GET
            if (isset($_GET['id'])) {
              $id = $_GET['id'];

              // Préparez votre requête SQL pour récupérer les détails du QCM
              ...
              if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                // Affichez les détails du QCM dans un formulaire pour la confirmation de la suppression
                ?>
                <h2>Voulez-vous vraiment supprimer le QCM suivant ?</h2>
                <table class="table table-striped table-bordered">
                  <thead>
                    <tr>
                      <th>Titre du QCM</th>
                      <th>Description du QCM</th>
                      <th>Solution du QCM</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td><?php echo $row['Titre_QCM']; ?></td>
                      <td><?php echo $row['Description_QCM']; ?></td>
                      <td><?php echo $row['Solution_QCM']; ?></td>
                    </tr>
                  </tbody>
                </table>
                <form action="delete_confirm_qcm.php" method="POST">
                  <input type="hidden" name="id" value="<?php echo $row['Id_QCM']; ?>">
